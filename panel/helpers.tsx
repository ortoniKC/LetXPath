...
export const colorizeCode = (code: string, _lang: string): React.ReactNode => {
  ...
  return tokens.map((token, idx) => {
    ...
    if (token.startsWith("//") || token.startsWith("#")) {
      ...
      /^(?:await|async|import|from|const|let|class|public|static|void|try|using|new|describe|it|def|return)$/.test(
        token,
      )
    ) {
...
export function generateRecordedScript(
      ...
      if (currentLang === "playwrightJS") {
        ...
        if (!locator.startsWith("page.")) {
          if (
            ...
            locator.startsWith("frameLocator(")
          ) {
            call = "page." + locator;
          } else {
        ...
        if (step.action === "click") {
          ...
          return (
            "  await expect(page." +
            (locator.startsWith("page.") ? locator.substring(5) : locator) +
            ").toBeVisible();"
          );
        }
        ...
        if (step.action === "click") {
          ...
          return (
            "    await element(by.xpath('" + xpath + "')).sendKeys('" + (step.value || "") + "');"
          );
        } else if (step.action === "assert_visible") {
        ...
        if (step.action === "click") {
          ...
          return (
            "    " +
            getCustomSnippet("sendKeys", "XPath", xpath, variable, method, templates).replaceAll(
              "${value}",
              step.value || "",
            )
          );
  ...
  switch (currentLang) {
    ...
    case "custom":
      return (
        "// Custom Framework Recorded Script\n// Start URL: " + initialUrl + "\n\n" + formattedSteps
      );
    default:
