...
export function filterAttributesFromElement(item: Attr): boolean {
  return (
    ...
    item.name === "jsmodel" ||
    item.name === "jsdata" ||
    item.name === "jscontroller" ||
    item.name === "face" ||
    item.name.includes("pattern") ||
    item.name.includes("length") ||
    item.name === "border" ||
    item.name === "formnovalidate" ||
    ...
    item.name === "ac_order_by" ||
    item.name.startsWith("data-ember") ||
    item.name === "href" ||
    item.name === "aria-autocomplete" ||
    item.name === "autocapitalize" ||
    item.name === "jsaction" ||
    item.name === "autocorrect" ||
    item.name === "aria-haspopup" ||
    item.name === "style" ||
    item.name === "size" ||
    item.name === "height" ||
    item.name === "width" ||
    ...
    item.name === "ng-model-update-on-enter" ||
    item.name === "magellan-navigation-filter" ||
    item.name === "ng-blur" ||
    item.name === "ng-focus" ||
    item.name === "ng-trim" ||
...
export function getNumberOfXPath(element: string): number | undefined {
  try {
    return state.elementOwnerDocument.evaluate(
      "count(" + element + ")",
      state.elementOwnerDocument,
      null,
      XPathResult.ANY_TYPE,
...
export function checkIDNameClassHref(parent: HTMLElement, bo: boolean): boolean {
  if (parent && parent.attributes) {
    Array.prototype.slice.call(parent.attributes).forEach(function (item: Attr) {
      if (item.name === "id" || item.name === "class" || item.name === "name") bo = true;
    });
  }
  return bo;
}
