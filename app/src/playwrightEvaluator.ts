...
function matchesParsedCSSClauses(el: HTMLElement, clauses: SelectorClause[], index: number): boolean {
...
function evaluateSelectorPart(part: any, doc: Document): HTMLElement[] {
  if (part.name === 'css') {
    ...
    return all.filter(el => {
      try {
        const selectorString = typeof part.body === 'string'
          ? part.body
          : (part.source || serializeSelector(part.body));
...
export function evaluatePlaywrightLocator(locatorStr: string, doc: Document): HTMLElement[] {
  ...
  const operations: { type: 'part' | 'nth' | 'first' | 'last' | 'frame-locator' | 'frame'; data?: any }[] = [];
