  ...
  try {
    ...
    if (element.previousElementSibling) {
      ep =
        ...
        element.previousElementSibling.textContent.length >= 2
          ...
          if (innerChildLen >= 1) {
            for (j = 0; j < innerChildLen; j++) {
              ...
              if (
                ...
                innerChild.textContent.length > 1 &&
                child[0].children[0].tagName != "OPTION"
              ) {
                ep = innerChild;
                setBool = true;
                break;
...
export function getVariableAndMethodName(str: string): void {
  ...
  state.methodName = temp
    .replace(/\W/g, "")
    .replace(/\d/g, "")
    .replace(/[&\/\\#,+()$~%.'":*?<>{}_-]/g, "");
  if (state.methodName.length > 0) {
    state.variableName =
      state.methodName.charAt(0).toLowerCase() +
      state.methodName.substring(1, state.methodName.length);
  } else {
