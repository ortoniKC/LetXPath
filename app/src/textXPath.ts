...
export function handleComplexText(element: HTMLElement, tagName: string, textContent: string): string | null {
    const xpaths = [
...
export function getNumberOfTextXPath(xpath: string): number {
    try {
        const result = state.elementOwnerDocument.evaluate(xpath, state.elementOwnerDocument, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
